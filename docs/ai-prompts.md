...
## <What you were trying to achieve>
To generate a `seed_demo_data` management command that populates the database with realistic, varied demo data — covering multiple events, sessions with different capacities, staff assignments across events, and registrations across every status (reserved, confirmed, checked_in, expired, cancelled) — so the application is not an empty shell when run for the first time.

### Prompt
6. Follow-up prompt to the master prompt to create the `seed_demo_data` management command under Phase 12 of `implementation_guide.md`.

### What you got
A working management command (`python manage.py seed_demo_data`) that creates organizer and staff users, multiple events (including at least one archived), sessions with varied capacities, staff assignments covering different scenarios (a staff member assigned across multiple events, a session with multiple staff, and one with no staff), registrations across every status with corresponding `RegistrationEvent` audit rows, and `DismissedAlert` rows on sessions that are not currently at capacity — all generated by calling `reserve_seat()` and `transition()` so the audit trail is produced the same way production data would be.

### What you corrected
The initial version created `Registration` rows directly with hardcoded statuses instead of going through the `reserve_seat()` and `transition()` service functions. This meant `RegistrationEvent` audit rows were missing for many registrations, and the lifecycle was inconsistent with how the application actually processes data. I corrected this by refactoring the seed command to call `reserve_seat()` for creating reservations and `transition()` for status changes, so every registration has a complete, correct audit trail generated through the same code path production data would use.
